...
from src.util import read_as_3d_array
from src.data import transforms


class ShapeNetDataset(torch.utils.data.dataset.Dataset):

    def __init__(self, cfg: DictConfig, dataset_type, transforms, image_process, opts):
    ...
    def __getitem__(self, idx: int) -> Tuple[np.ndarray, np.ndarray, tuple]:
        ...
        if self.transforms:
            ...
            aug_images = self.transforms(tmp)
        
        images = self.image_process(images)

        return images, aug_images, volume, taxonomy_name, sample_name
