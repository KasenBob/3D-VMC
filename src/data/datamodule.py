...
class ShapeNetDataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        ...
        pre_image_process = self.get_pre_image(cfg_dt)
        # Set up data augmentation
        train_transforms    = self.get_train_transforms(cfg_dt)
        # Initialize dataset
        train_dataset       = ShapeNetDataset(cfg_ds, 'train', train_transforms, pre_image_process, self.dataset_opts)
        # Initialize dataloader
        train_data_loader = torch.utils.data.DataLoader(
            dataset=train_dataset,
            batch_size=cfg_dl.train_batch_size,
    ...
    def val_dataloader(self):
        ...
        # Initialize dataset
        val_dataset = ShapeNetDataset(cfg_ds, 'val', val_transforms, pre_image_process, self.dataset_opts)

        # Initialize dataloader
        val_data_loader = torch.utils.data.DataLoader(
            dataset=val_dataset,
            batch_size=cfg_dl.val_batch_size,
            num_workers=cfg_dl.num_workers,
            pin_memory=True,
            shuffle=False)
    ...
    def test_dataloader(self):
        ...
        test_transforms = self.get_eval_transforms(cfg_dt)
        # Initialize dataset
        test_dataset = ShapeNetDataset(cfg_ds, 'test', test_transforms, pre_image_process,  self.dataset_opts)
        # Initialize dataloader
        test_data_loader = torch.utils.data.DataLoader(
            dataset=test_dataset,
            batch_size=cfg_dl.test_batch_size,
            num_workers=0,
            pin_memory=True,
            shuffle=False)
    ...
    def get_pre_image(self, cfg_dt: DictConfig) -> transforms.Compose:
